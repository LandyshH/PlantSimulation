...
public sealed class StrawberryStemAnimationSystem : IEcsRunSystem
{
    ...
    public void Run()
    {
        ...
        foreach (var j in _leafFilter)
        {
            ...
            leaf.LeafGO.transform.position = Vector3.Lerp(leaf.LeafGO.transform.position, targetPosition, 0.3f * Time.deltaTime);

            Vector3 leafMaxScale = new Vector3(1f, leaf.MaxHeight, leaf.MaxWidth);
